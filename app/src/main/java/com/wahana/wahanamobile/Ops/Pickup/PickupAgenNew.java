...
public class PickupAgenNew extends DrawerHelper {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //get force close
        Thread.setDefaultUncaughtExceptionHandler(new UnCaughtException(PickupAgenNew.this));
...
//        });
        ...
        db = new DatabaseHandler(PickupAgenNew.this);
        db.deleteKode();
        progressDialog = new ProgressDialog(PickupAgenNew.this, R.style.AppTheme_Dark_Dialog);
        session = new SessionManager(PickupAgenNew.this);
...
//        Log.d("postreq",""+session.getID()+"|"+agentcode+"|"+session.getID()+"|"+rc);
        ...
        Call<ApiPickup> result = mApiInterface.pickupBeginAccount(session.getSessionID(),agentcode,session.getID(),rc);
            ...
            public void onResponse(Call<ApiPickup> call, Response<ApiPickup> response) {
                ...
                if(response.isSuccessful()) {
                    ...
                    try {
                        ...
                        if (vrcode.equals("1")) {
                            Toast.makeText(PickupAgenNew.this, " response version " + response.body().getVerificationCode(), Toast.LENGTH_SHORT).show();

                            Log.d("retro", "" + response.body().getVerificationCode() + "|" + vurl + "|" + verificationcode);

                            DatabaseHandler db = new DatabaseHandler(PickupAgenNew.this);
                            db.addAgent(new AgentCode(agentcode));
                            Intent intent = new Intent(PickupAgenNew.this, PickupKodeVerifikasiNew.class);
                            ...
                            Toast.makeText(PickupAgenNew.this, " error " + vrmsg, Toast.LENGTH_SHORT).show();
                        }
                    ...
                    try {
                        Toast.makeText(PickupAgenNew.this, " error " + response.errorBody().string(), Toast.LENGTH_SHORT).show();
                    } catch (IOException e) {
                        Toast.makeText(PickupAgenNew.this, " unknown error", Toast.LENGTH_SHORT).show();
                        e.printStackTrace();
                    }
            ...
            protected void onPostExecute(SoapObject result) {
                ...
                if(result==null){
                    runOnUiThread(new Runnable() {
                        public void run() {
                            Toast.makeText(PickupAgenNew.this, getString(R.string.error_message), Toast.LENGTH_LONG).show();
                            progressDialog.dismiss();
                        }
                    });
                }else {
                    try{
                        ...
                        if (text.equals("OK")) {
                            ...
                            runOnUiThread(new Runnable() {
                                public void run() {
                                    Toast.makeText(PickupAgenNew.this, text,Toast.LENGTH_LONG).show();
                                }
                            });
                        }
                    }catch (Exception e){
